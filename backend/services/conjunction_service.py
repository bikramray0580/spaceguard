...
def screen_catalogue(
...
) -> tuple[tuple[str, ...], int, int, ScreeningResult]:
    """Run broad_screen() over catalogue objects and return candidate pairs.

    The existing pairwise screen_conjunction() path is unchanged. This only
    wires the already-implemented coarse screen onto ConjunctionPropagator.
    """
    if end <= start:
        raise ValueError("end must be later than start")
    if step_minutes <= 0 or step_minutes > 60:
    ...
    return selected_ids, catalogue_count, pair_count, result
