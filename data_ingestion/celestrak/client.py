...
_FALLBACK_CANDIDATES = (
    ...
    Path(__file__).resolve().parents[2] / "shared" / "data" / "sample" / "celestrak_fallback.json",
)
...
def fetch_celestrak_data() -> list[dict]:
    """Download current GP records from CelesTrak.

    Returns a list of GP JSON dictionaries. If the live request fails after
    limited retries, a clearly labeled local development sample is returned.
    """
    ...
    try:
        records = _download_gp_json(url=url, group=group, fmt=fmt)
    except CelesTrakFetchError as exc:
        print("CelesTrak unavailable.")
        print(f"Reason: {exc}")
        print("Using local sample data for development.")
        return _load_fallback_sample()
    print(f"Fetched {len(records)} records from CelesTrak (GROUP={group}, FORMAT={fmt}).")
    return records
...
def _download_gp_json(*, url: str, group: str, fmt: str) -> list[dict]:
    ...
    response = _get_with_retries(url, params=params, timeout=timeout, retries=retries)
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            last_error = CelesTrakFetchError(
                ...
                f"connection/read timed out after {timeout} on attempt {attempt}/{attempts}"
            )
            last_error.__cause__ = exc
        except ConnectionError as exc:
            last_error = CelesTrakFetchError(f"{exc}")
            ...
            if response.status_code in _RETRY_STATUSES:
                last_error = CelesTrakFetchError(
                    f"HTTP {response.status_code} from CelesTrak on attempt {attempt}/{attempts}"
                )
        ...
        if attempt < attempts:
            wait_s = 2 ** (attempt - 1)
            print(f"Attempt {attempt}/{attempts} failed ({last_error}). Retrying in {wait_s}s...")
            time.sleep(wait_s)
            continue
        raise last_error if last_error else CelesTrakFetchError("CelesTrak request failed")
...
def _parse_gp_payload(text: str) -> list[dict]:
    ...
    try:
        ...
        raise CelesTrakFetchError(f"CelesTrak returned malformed JSON: {exc}") from exc
    ...
    for path in _FALLBACK_CANDIDATES:
        if path.is_file():
            ...
            for item in records:
                ...
                item["SOURCE_NOTE"] = "development fallback; not live CelesTrak data"
            if records:
                return records
            break
    raise CelesTrakFetchError(
